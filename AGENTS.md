...
- Correctness guarantees cover Git operations initiated through the app, not concurrent mutations from terminals or other external Git processes. Treat external Git activity as out-of-band: do not add locks, repeated admission checks, or transaction-style recovery to make app operations atomic against it. Later authoritative repo reads must converge to the resulting Git state.
