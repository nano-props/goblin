import { describe, expect, test } from 'vitest'
import {
  formatTerminalSlotKey,
  formatWorktreeKey,
  parseTerminalSlotKey,
  parseWorktreeKey,
  terminalPruneKeyFromSlotKey,
} from '#/shared/terminal-slot-key.ts'

describe('terminal session key helpers', () => {
  test('formats and parses terminal session keys round-trip', () => {
    const key = formatTerminalSlotKey('/repo', '/repo/worktree', 'terminal-2')
    expect(key).toBe('/repo\0/repo/worktree\0terminal-2')
    expect(parseTerminalSlotKey(key)).toEqual({
      repoRoot: '/repo',
      worktreePath: '/repo/worktree',
      slotId: 'terminal-2',
    })
  })

  test('rejects malformed terminal session keys', () => {
    expect(parseTerminalSlotKey('')).toBeNull()
    expect(parseTerminalSlotKey('/repo')).toBeNull()
    expect(parseTerminalSlotKey('/repo\0/worktree')).toBeNull()
    expect(parseTerminalSlotKey('/repo\0/worktree\0')).toBeNull()
    expect(parseTerminalSlotKey('\0/worktree\0terminal-1')).toBeNull()
  })

  test('formats and parses worktree terminal keys round-trip', () => {
    const key = formatWorktreeKey('/repo', '/repo/worktree')
    expect(key).toBe('/repo\0/repo/worktree')
    expect(parseWorktreeKey(key)).toEqual({
      repoRoot: '/repo',
      worktreePath: '/repo/worktree',
    })
  })

  test('rejects malformed worktree terminal keys', () => {
    expect(parseWorktreeKey('')).toBeNull()
    expect(parseWorktreeKey('/repo')).toBeNull()
    expect(parseWorktreeKey('/repo\0')).toBeNull()
    expect(parseWorktreeKey('\0/worktree')).toBeNull()
    expect(parseWorktreeKey('/repo\0/worktree\0terminal-1')).toBeNull()
  })

  test('builds prune keys from valid session keys only', () => {
    expect(terminalPruneKeyFromSlotKey('/repo\0/repo/worktree\0terminal-3')).toBe('/repo\0/repo/worktree')
    expect(terminalPruneKeyFromSlotKey('invalid')).toBeNull()
  })
})
