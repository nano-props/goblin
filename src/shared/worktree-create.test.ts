...
describe('worktree create helpers', () => {
  ...
  test('applies wildcard and exact negative fetch refspecs to source refs', () => {
    expect(
      parseRemoteTrackingRefs('refs/remotes/origin/main\n', [
        {
          name: 'origin',
          fetchSpecs: ['+refs/heads/*:refs/remotes/origin/*', '^refs/heads/archive/*', '^refs/heads/private'],
        },
      ]),
  ...
  test.each(['+refs/pull/**/head:refs/remotes/origin-pr/*', 'refs/notes/*:refs/notes/**', 'refs/tags/..'])(
    'rejects malformed positive refspecs even when they do not produce remote branches: %s',
    (spec) => {
      expect(() =>
        parseRemoteTrackingRefs('refs/remotes/origin/main\n', [
          {
