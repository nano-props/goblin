...
async function notifyTerminalBell(webContents: WebContents, input: TerminalNotifyBellInput): Promise<boolean> {
  ...
  try {
    ...
    return await showNotificationWithResult(input.title, input.body, input.repoRoot, input.terminalKey)
  } catch (err) {
...
function showNotificationWithResult(
  ...
  return new Promise((resolve) => {
    ...
    notif.once('click', () => {
      ...
      if (repoRoot) broadcastClientEffectIntent({ type: 'terminal-bell-click', repoRoot, terminalKey })
    })
    notif.show()
  })
}
