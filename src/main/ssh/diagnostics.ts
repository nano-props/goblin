...
const STAGE_LABELS: Record<RemoteDiagnosticStageName, string> = {
  ssh: 'SSH reachable',
  shell: 'Shell available',
  git: 'Git installed',
  path: 'Path exists',
  repo: 'Git repository valid',
}
...
export async function testRemoteRepository(
  ...
  const fail = (
    ...
    stages[index] = {
      ...
      status: 'failed',
      category,
      message: result.message || category,
...
function createStages(): RemoteDiagnosticStage[] {
  return (Object.keys(STAGE_LABELS) as RemoteDiagnosticStageName[]).map((name) => ({
    name,
