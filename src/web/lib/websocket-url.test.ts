...
describe('server URL resolution', () => {
  test('preserves the authoritative server origin instead of substituting the page origin', async () => {
    expect(resolveApiBaseUrl('http://127.0.0.1:32101/')).toBe('http://127.0.0.1:32101/')
  })

  test('derives the WebSocket protocol from the authoritative server URL', async () => {
