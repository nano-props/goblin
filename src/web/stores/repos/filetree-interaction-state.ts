...
export function filetreeInteractionScopeKey(repoId: string, worktreePath: string): string {
  return `${repoId}\0${worktreePath}`
}

export function parseFiletreeInteractionScopeKey(
  scopeKey: string,
): { readonly repoId: string; readonly worktreePath: string } | null {
  const parts = scopeKey.split('\0')
  if (parts.length !== 2) return null
  const [repoId, worktreePath] = parts
  return repoId && worktreePath ? { repoId, worktreePath } : null
}
