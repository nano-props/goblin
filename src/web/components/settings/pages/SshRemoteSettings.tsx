      ...
      <SettingsGroup label={t('settings.ssh.title')} hint={t('settings.ssh.body')}>
        <SettingsList>
          <SettingsRow
            ...
            control={<span class="font-mono text-[11px] text-muted-foreground">~/.ssh/config</span>}
          />
          <SettingsRow
            controlId="settings-ssh-path-format"
            label={t('settings.ssh.path-format-label')}
            hint={t('settings.ssh.path-format-hint')}
            control={<span class="font-mono text-[11px] text-muted-foreground">/srv/repo · ~/repo</span>}
          />
        </SettingsList>
        <SettingsCard>
          <pre class="overflow-x-auto whitespace-pre-wrap px-4 py-3 font-mono text-[11px] leading-snug text-muted-foreground">
