...
interface RepoDetailsTooltipLayerProps extends ComponentPropsWithoutRef<'div'> {
  repos: RepoPickerRepo[]
  delayMs?: number
}

const REPO_DETAILS_TOOLTIP_DELAY_MS = 100

const REPO_DETAILS_TOOLTIP_SELECTOR = '[data-repo-tooltip-id]'
const REPO_DETAILS_TOOLTIP_META_TEXT_CLASS = 'text-xs leading-4 text-muted-foreground'
const REPO_DETAILS_TOOLTIP_ICON_CLASS = 'shrink-0 text-muted-foreground/80'

export function RepoDetailsTooltipLayer({
  repos,
  delayMs = REPO_DETAILS_TOOLTIP_DELAY_MS,
  children,
  ...props
  ...
  return (
    <DelegatedTooltipLayer
      items={repos}
      selector={REPO_DETAILS_TOOLTIP_SELECTOR}
      attributeName="data-repo-tooltip-id"
      getItemId={(repo) => repo.id}
      renderTooltip={(repo) => <RepoDetailsTooltipContent repo={repo} />}
      delayMs={delayMs}
...
function RepoDetailsTooltipContent({ repo }: { repo: RepoPickerRepo }) {
  const t = useT()
  ...
  const syncedAtLabel = syncedAtIso ? formatRelativeTime(syncedAtIso, lang) : t('repo-picker.tooltip.not-synced')
  return (
    <>
      <div className="truncate text-xs font-semibold text-foreground">{repo.name}</div>
      <div className={cn('mt-0.5 truncate font-mono', REPO_DETAILS_TOOLTIP_META_TEXT_CLASS)}>
        {formatRepoLocator(repo.id, remoteTarget)}
      </div>
      <div className={cn('mt-1 flex min-w-0 items-center gap-1.5', REPO_DETAILS_TOOLTIP_META_TEXT_CLASS)}>
        <span className="shrink-0">{t('repo-picker.tooltip.last-sync-label')}</span>
        {syncedAtIso ? (
        ...
        <div className={cn('mt-2 border-t border-border/40 pt-2', TOOLTIP_STACK_MD_CLASS)}>
          {repo.remoteDetails.map((remote) => {
            ...
            return sameUrl ? (
              <div
                key={remote.name}
                className={cn('flex min-w-0 items-center gap-1.5', REPO_DETAILS_TOOLTIP_META_TEXT_CLASS)}
              >
                <span className="shrink-0 font-mono">{remote.name}</span>
                <span className={cn('font-mono', REPO_DETAILS_TOOLTIP_ICON_CLASS)} aria-hidden>
                  →
                </span>
                <span className="min-w-0 truncate font-mono">{remote.fetchUrl}</span>
                <ArrowUpDown size={10} className={REPO_DETAILS_TOOLTIP_ICON_CLASS} aria-hidden />
              </div>
            ) : (
              <div key={remote.name} className={cn(TOOLTIP_STACK_SM_CLASS, REPO_DETAILS_TOOLTIP_META_TEXT_CLASS)}>
                <div className="font-mono">{remote.name}</div>
                <div className="flex min-w-0 items-center gap-1 pl-1">
                  <ArrowDown size={10} className={REPO_DETAILS_TOOLTIP_ICON_CLASS} aria-hidden />
                  <span className="min-w-0 truncate font-mono">{remote.fetchUrl}</span>
                </div>
                <div className="flex min-w-0 items-center gap-1 pl-1">
                  <ArrowUp size={10} className={REPO_DETAILS_TOOLTIP_ICON_CLASS} aria-hidden />
      ...
      {repo.remoteDetails.length === 0 && (
        <div className={cn('mt-2 border-t border-border/40 pt-2', REPO_DETAILS_TOOLTIP_META_TEXT_CLASS)}>
          {t('repo-picker.tooltip.no-remotes')}
        </div>
      )}
    </>
  )
