...
describe('terminalViewportText', () => {
  test('joins soft-wrapped rows and preserves hard line breaks and internal blanks', async () => {
    expect(viewport([line('first'), line(' continued', true), line(''), line('last')])).toBe('first continued\n\nlast')
  })

  test('trims empty rows only from the bottom of the visible viewport', async () => {
    expect(viewport([line('output'), line(''), line('')])).toBe('output')
    expect(viewport([line(''), line('')])).toBe('')
  })

  test('starts at viewportY without inventing a leading newline for a wrapped first row', async () => {
    expect(viewport([line('above'), line('visible continuation', true), line('next')], 2, 80, 1)).toBe(
      'visible continuation\nnext',
    )
  })

  test('bounds translation to the current columns and normalizes xterm non-breaking spaces', async () => {
  ...
  test('treats unavailable visible rows as blank rows', async () => {
