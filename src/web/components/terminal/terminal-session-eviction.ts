export function countOrphanedTerminalSessionIds(input: {
  ...
  getRepoRuntimeIdForTerminalSessionId: (terminalSessionId: string) => string | null
  ...
  for (const terminalSessionId of input.localTerminalSessionIds) {
    ...
    if (input.getRepoRuntimeIdForTerminalSessionId(terminalSessionId) !== input.repoRuntimeId) continue
