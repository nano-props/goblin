...
describe('compactTerminalTitle', () => {
  test('prefers the trailing command segment from long terminal titles', async () => {
    expect(compactTerminalTitle('~/Developer/goblin — npm run dev')).toBe('goblin · npm run dev')
  })

  test('reduces host path titles to host and basename', async () => {
    expect(compactTerminalTitle('prod:~/services/payments/api')).toBe('prod · api')
  })

  test('keeps useful context for host path and command titles', async () => {
    expect(compactTerminalTitle('prod:~/services/payments/api — npm run dev')).toBe('prod · api · npm run dev')
  })

  test('extracts the basename from paths that contain spaces', async () => {
    expect(compactTerminalTitle('~/Library/Application Support/Goblin')).toBe('Goblin')
  })

  test('shortens long commands even when they only contain one or two tokens', async () => {
  ...
  test('does not mistake urls for host path titles', async () => {
    expect(compactTerminalTitle('https://example.com/very/long/path/to/page')).toBe('page')
  })

  test('strips leading labels like devin before compacting the real title', async () => {
    expect(compactTerminalTitle('devin: some real info')).toBe('some real info')
  })

  test('strips ubuntu@VM host prefix before compacting the real title', async () => {
  ...
  test('handles nested terminal title wrappers from real remote sessions', async () => {
  ...
  test('keeps readable short names for real ssh and command titles', async () => {
...
describe('compactTerminalProcessName', () => {
  test('compacts shell executable paths to their basename', async () => {
  ...
  test('keeps plain process names unchanged', async () => {
