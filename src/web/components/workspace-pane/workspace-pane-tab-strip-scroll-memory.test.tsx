...
beforeEach(() => {
  Object.defineProperty(HTMLElement.prototype, 'getBoundingClientRect', {
    ...
    value(this: HTMLElement) {
      if (this.matches('[data-reka-scroll-area-viewport]')) return rect(0, 200)
...
describe('workspace pane tab strip scroll memory', () => {
  test('restores the exact position after the tab strip unmounts', async () => {
    ...
    await flushTestUpdates(() => firstViewport.dispatchEvent(new Event('scroll')))
  ...
  test('positions a tab that becomes active after mounting', async () => {
    const result = renderInJsdom(
      <WorkspacePaneTabStripScrollMemoryProvider>
...
function viewport(): HTMLDivElement {
  const element = document.body.querySelector<HTMLDivElement>('[data-reka-scroll-area-viewport]')
