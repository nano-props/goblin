...
import type { TerminalComposerHandle, TerminalComposerLabels } from '#/web/terminal/components/terminal-composer.tsx'
import { isImeOwnedKeyboardEvent, isMacNavigatorPlatform } from '#/web/terminal/components/terminal-keyboard.ts'
import {
  AttachmentOverlay,
  EmptyTerminalCta,
  PresentationFailureOverlay,
  StatusOverlay,
} from '#/web/terminal/components/terminal-session-overlays.tsx'
...
const TERMINAL_PRESENTATION_PENDING_OPERATION_LABEL_KEYS = {
...
} as const satisfies Record<TerminalPresentationPendingOperation, DictKey>
  ...
  setup(props) {
    ...
    function terminalComposerLabels(): TerminalComposerLabels {
      return {
        ...
        sendOnly: t('terminal.composer-send-only'),
    ...
    async function handleComposerSend(text: string): Promise<boolean> {
      ...
      return context.sendComposerText(sessionId, text)
    }

    async function handleComposerSubmit(text: string): Promise<boolean> {
      const sessionId = terminalSessionId.value
      if (!sessionId || !isController.value || !text) return false
      return context.submitComposerText(sessionId, text)
    }
        ...
        >
          ...
          {controller && currentSessionId ? (
            <TerminalComposer
              ...
              onSubmitText={handleComposerSubmit}
