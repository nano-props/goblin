...
export interface RealtimeBrokerOptions {
  ...
  livenessTimeoutReason?: string
  shutdownReason?: string
}

/**
 * The client sends a health ping every `REALTIME_LIVENESS_PROBE_INTERVAL_MS`
 * (30 s). If no ping refreshes a socket for
 * `REALTIME_LIVENESS_DEADLINE_MS` (90 s), that exact transport is closed.
 * Client presence remains online while any socket for that client remains.
 *
 * The actual silence detection latency is at most
 * `REALTIME_LIVENESS_DEADLINE_MS + REALTIME_LIVENESS_PROBE_INTERVAL_MS` = 120 s
 * because scans run at the probe interval.
 */
export const REALTIME_LIVENESS_PROBE_INTERVAL_MS = 30_000
export const REALTIME_LIVENESS_DEADLINE_MS = 90_000
...
export class RealtimeBroker<TMessage> {
  ...
  constructor(options: RealtimeBrokerOptions) {
    this.options = {
      livenessTimeoutReason: 'realtime liveness timeout',
      shutdownReason: 'server shutting down',
      ...options,
    }
    this.livenessTimer = setInterval(() => this.scanLiveness(), REALTIME_LIVENESS_PROBE_INTERVAL_MS)
  ...
  registerSocket(clientId: string, userId: string, socket: RealtimeSocket): void {
    ...
    sockets.add(socket)
    this.socketMetaBySocket.set(socket, { clientId, userId, lastLivenessAt: Date.now() })
  ...
  /** Update the last-liveness receipt time for the socket that sent a health ping. */
  recordLiveness(socket: RealtimeSocket): void {
    const meta = this.socketMetaBySocket.get(socket)
    if (meta) meta.lastLivenessAt = Date.now()
  }

  private scanLiveness(): void {
    const now = Date.now()
    for (const [socket, meta] of Array.from(this.socketMetaBySocket)) {
      if (now - meta.lastLivenessAt < REALTIME_LIVENESS_DEADLINE_MS) continue
      this.closeSocket(socket, 1001, this.options.livenessTimeoutReason)
    }
  }
