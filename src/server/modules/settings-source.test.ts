...
describe('settings source', () => {
  ...
  test('serializes concurrent settings mutations without dropping updates', async () => {
    ...
    expect(await mod.getServerRecentWorkspaces()).toEqual([{ id: REPO_C }, { id: REPO_B }, { id: REPO_A }])
  ...
  test('does not invalidate runtimes when durable workspace membership is only reordered', async () => {
    ...
    try {
      ...
      await expect(
        source.compareAndReplaceServerWorkspaceEntries([repoA, repoB], [repoB, repoA]),
