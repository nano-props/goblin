...
describe('deriveUserId', () => {
  it('returns a stable id for the same token', async () => {
    const a = deriveUserId('token-a')
    const b = deriveUserId('token-a')
    expect(a).toBe(b)
  })

  it('returns different ids for different tokens', async () => {
    const a = deriveUserId('token-a')
    const b = deriveUserId('token-b')
    expect(a).not.toBe(b)
  })

  it('prefixes the id with `user_` so log lines and Map keys are unambiguous', async () => {
    expect(deriveUserId('token-c').startsWith('user_')).toBe(true)
  })

  it('produces ids of the same length regardless of input length', async () => {
  ...
  it('does not throw on an empty token (returns the empty-token hash)', async () => {
