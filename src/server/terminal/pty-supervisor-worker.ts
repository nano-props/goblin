...
export class WorkerBackedPtySupervisor implements PtySupervisor {
  ...
  async spawn(input: PtySpawnInput): Promise<PtySpawnResult> {
    ...
    return await new Promise<PtySpawnResult>((resolve) => {
      const pending: PendingSpawn = { input, handle, ownership, resolve }
  ...
  kill(handle: PtyHandle): void {
    ...
    this.worker?.send({ type: 'pty-kill', ptySessionId: handle.ptySessionId })
  }
  ...
  async killAndWait(handle: PtyHandle): Promise<void> {
    ...
    const worker = this.worker
    if (!worker) return
    worker.send({ type: 'pty-kill', ptySessionId: handle.ptySessionId })
  ...
  shutdown(): void {
    ...
    this.pendingSpawnsByPtySessionId.clear()
  ...
  private handleWorkerMessage(worker: TerminalWorkerChildProcess, message: PtyWorkerMessage): void {
    ...
    if (message.type === 'pty-spawn-result') {
      ...
      if (message.ok) {
        ...
        this.pendingSpawnsByPtySessionId.delete(message.ptySessionId)
        if (message.processName !== 'terminal' || pending.ownership.processName === 'terminal') {
          pending.ownership.processName = message.processName
        }
        const processName = pending.ownership.processName
        if (!pending.ownership.exitCompletion.completed) this.sessions.set(message.ptySessionId, pending.ownership)
        pending.resolve({
          ok: true,
          handle: pending.handle,
          processName,
          events: pending.ownership.channel.lease,
        })
      } else {
        this.pendingSpawns.delete(message.requestId)
        this.releasePendingSpawn(pending)
  ...
  private sendSpawnRequest(requestId: string, pending: PendingSpawn): void {
    try {
      ...
      // ChildProcess.send() returning false only reports IPC backpressure. The
      // callback reports an actual delivery failure; otherwise the worker's
      // spawn result remains authoritative.
      worker.send(
        {
          type: 'pty-spawn',
          requestId,
        ...
        (error) => {
          if (!error || this.pendingSpawns.get(requestId) !== pending) return
  ...
  private releasePendingSpawn(pending: PendingSpawn): void {
