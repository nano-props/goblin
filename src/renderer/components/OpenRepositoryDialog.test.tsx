...
beforeEach(() => {
  ...
  testWindow.goblin = {
    ...
    invokeRpc: async (request: { path: string; input?: unknown }) => {
      ...
      if (request.path === 'repo.openDialog') return '/Users/tester/Developer/repo'
      return null
    },
    abortRpc: async () => true,
    onEvent: () => () => {},
...
describe('OpenRepositoryDialog', () => {
  ...
  test('can fill the path from the native picker and keeps the dialog open on failure', async () => {
    ...
    await flush()
    expect(rpcCalls).toContainEqual(expect.objectContaining({ path: 'repo.openDialog', input: undefined }))
