...
import { failedDiagnosticsCategory, remoteSshCommand, shouldOfferSshSettings } from '#/renderer/lib/remote-support.ts'
import { rpc } from '#/renderer/rpc.ts'
import { useT } from '#/renderer/stores/i18n.ts'
import type { RemoteDiagnosticCategory, RemoteDiagnosticStageName, RemoteDiagnosticsResult } from '#/shared/remote-repo.ts'
...
export function RemoteDiagnosticsPanel({ diagnostics, loading, onRetry }: Props) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border/60 bg-background/85 shadow-[var(--shadow-inset-highlight)]">
      ...
      <div className="space-y-2 px-3 py-3">
        {summaryTarget && (
          <div className="rounded-md border border-border/60 bg-muted/20 px-2.5 py-2 text-[11px] text-muted-foreground">
            ...
            <div className="mt-0.5">
              {t('repo-tabs.open-remote-host-label')}: {summaryTarget.host} · {t('repo-tabs.open-remote-username-label')}:{' '}
              {summaryTarget.user} · {t('repo-tabs.open-remote-port-label')}: {summaryTarget.port}
            </div>
          </div>
        )}
        {diagnostics?.stages.map((stage) => (
          <div key={stage.name} className="rounded-md border border-border/50 bg-background/60 px-2.5 py-2">
        ...
        {diagnostics?.details && !diagnostics.ok && (
          <div className="whitespace-pre-wrap break-words rounded-md border border-border/50 bg-muted/20 px-2.5 py-2 font-mono text-[11px] text-muted-foreground">
            {diagnostics.details}
          </div>
        )}
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            {diagnostics.details && (
              <Button
                ...
                size="sm"
                variant="ghost"
                onClick={() => void copyText(diagnostics.details ?? '')}
              >
        ...
        {loading && <div className="px-0.5 py-1 text-xs text-muted-foreground">{t('repo-tabs.open-remote-diagnostics-testing')}</div>}
      </div>
    </div>
  )
}
