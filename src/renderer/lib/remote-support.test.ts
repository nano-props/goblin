...
import {
  ...
  unavailableBodyKey,
} from '#/renderer/lib/remote-support.ts'
...
describe('remote support helpers', () => {
  ...
  test('maps remote unavailable reasons to more specific body copy', () => {
    ...
    expect(unavailableBodyKey(true, 'repo-tabs.open-remote-home-unavailable')).toBe('repo-unavailable.remote-home-unavailable')
  ...
  test('translates remote diagnostic categories through the diagnostics dictionary keys', () => {
    const t = (key: string) =>
      key === 'repo-tabs.open-remote-diagnostics-category-auth-failed' ? 'Authentication failed' : key
