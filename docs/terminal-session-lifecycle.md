...
A fresh response has no output checkpoint. The client commits the new binding,
keeps realtime output queued without parsing while the fitted xterm is hidden,
and reveals after one full-viewport render and the final geometry check. It then
flushes queued output in transport order. Recovery instead replays the atomic
`snapshot`/`snapshotSeq` frame before that render. A current automatic focus
intent is fulfilled when either view is revealed. Snapshot-generated protocol
replies are discarded; replies generated by fresh live output follow the normal
presented, current-generation input path. Neither `snapshotSeq` nor first output
has lifecycle, focus, prompt-readiness, or shell-redraw semantics.
...
- stream frame: drop nothing, so every event reaches the client after its
  binding metadata is committed; the client queues it until presentation and
  then continues in the same ordered stream;
