...
- Keep renderer identity semantics aligned across web and Electron:
  - `ownerId`: authenticated terminal owner. The server partitions session visibility, lifecycle cleanup, and realtime fanout by this id.
  ...
  - `attachmentId`: specific renderer attachment/socket under an owner.
