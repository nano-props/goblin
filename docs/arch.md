...
- Server push should be the default way other clients converge after a successful write. When the initiating command needs canonical post-write state before it can navigate, return that state from the server mutation and accept it directly into the query cache. Do not assemble one command from a mutation, a client read-back, and an invalidation race.
