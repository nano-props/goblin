...
Workspace-pane static layout is owned by `WorkspacePaneLayoutRepository`; runtime placement, physical indexes, and epoch clocks are owned by `WorkspacePaneEpochOverlay`; live membership belongs to runtime providers. `src/server/workspace-pane/*` purely projects the three, materializing live entries and filtering stale placement hints only in the returned canonical view. Reads do not write derived membership, and restore never copies layout into runtime state. Terminal contributes one runtime provider; the client renders this projection rather than inventing fallback rules from local terminal views.
