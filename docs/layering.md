...
The server-side terminal runtime is created by `createServerTerminalRuntime({ ptySupervisor })` and is the only place that implements `ServerTerminalHost`. The realtime route receives the host via dependency injection from the server factory. The TerminalSessionProvider on the client side keeps `TerminalSessionProjection` as the single source of truth for live session state and uses the terminal client only for fetches and mutations.

Workspace-pane terminal tab membership is also server-owned. `listWorkspaceTabs` reconciles workspace-pane tabs against live terminal sessions, materializing missing terminal entries and pruning stale ones before returning the canonical projection. The client should render that projection rather than inventing terminal-tab fallback rules from local terminal views.
