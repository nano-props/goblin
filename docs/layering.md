...
Workspace-pane runtime tab membership is also server-owned, but it is not owned by the terminal feature. `src/server/workspace-pane/*` reconciles workspace-pane tabs against live runtime sessions, materializing missing runtime entries and pruning stale ones before returning the canonical projection. Terminal contributes one runtime provider; the client should render the workspace-pane projection rather than inventing terminal-tab fallback rules from local terminal views.
