...
- Git changes made from terminals or other external processes are out-of-band. The app does not guarantee that its Git operations are atomic against those concurrent changes, nor must it detect every external mutation as it happens. If external state prevents the happy path, fail the current operation directly and require the user to repair, retry, or reopen. The next focus, navigation, explicit refresh, successful background sync, or other authoritative repo read only needs to converge the client projection to the resulting Git state.
