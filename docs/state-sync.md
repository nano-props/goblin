...
- Git changes made from terminals or other external processes are out-of-band. The app does not guarantee that its Git operations are atomic against those concurrent changes, nor must it detect every external mutation as it happens. Keep lifecycle invalidation best-effort and let the next focus, navigation, explicit refresh, or other authoritative repo read converge the client projection to the resulting Git state; do not add polling, locks, or repeated admission checks solely to coordinate with external Git tools.
